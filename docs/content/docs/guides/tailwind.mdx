...
For Tailwind CSS v4, shape-morph ships a pre-built utility stylesheet. Import it alongside Tailwind and use `clip-{shape}` classes directly — no JavaScript needed.
...
The stylesheet contains `@utility` directives — the Tailwind v4 way to define custom utilities in CSS. Each utility sets `clip-path` to a pre-computed `polygon()` value. Because this is pure CSS, it works with all Tailwind features like responsive prefixes, hover states, and transitions.
