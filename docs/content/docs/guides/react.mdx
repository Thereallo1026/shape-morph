...
| `easing` | `(t: number) => number` | `easeInOut` | Easing function for duration-based animation. Cannot use with `lerp`. |
| `lerp` | `number` | - | Lerp factor (0-1). Each frame moves this fraction toward target. Cannot use with `duration` or `easing`. |
