...
For simpler animation setup, use [`AnimatedMorph`](/docs/api/core#animatedmorph) instead of a manual `requestAnimationFrame` loop:
...
`AnimatedMorph` handles the animation loop, easing, and cleanup internally. Set `progress` to trigger animation.
