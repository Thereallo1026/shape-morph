...
`toMorphPair` creates the morph internally and returns both the start and end `polygon()` strings as a tuple. Both strings have identical vertex counts, so CSS can transition between them directly.
...
For zero-JS utility classes, see the [Tailwind CSS](/docs/guides/tailwind) guide.
