...
Animated morph for vanilla JS. Set the `progress` property and the class animates from the current position, calling `onFrame` each frame. Supports duration-based (with customizable easing) and lerp-based animation — the same modes as the React [`useMorph`](/docs/api/react#usemorph) hook.
...
|---|---|---|---|
| `duration` | `number` | `300` | Animation duration in ms. Cannot use with `lerp`. |
| `easing` | `(t: number) => number` | `easeInOut` | Easing function. Cannot use with `lerp`. See [Easing](/docs/api/easing). |
| `lerp` | `number` | - | Lerp factor (0-1). Cannot use with `duration` or `easing`. |
